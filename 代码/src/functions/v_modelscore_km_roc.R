#' @TODO 根据模型计算得分，并且绘制分组km、ROC以及风险三联图
#' @title ### 模型效能分析
#' @param signature_coef 数据框，包含两列signature和coef，模型的signature以及对应的系数coef
#' @param od 结果输出路径
#' @param exp 包含signature的矩阵，signature在行样本在列（例如表达谱）
#' @param clin 样本临床信息，包含sample列并且sample列与exp列名是一致的
#' @param best_cut 是否最优分组，默认FALSE
#' @param dataset 字符向量，数据集名称，用于输出文件名
#' @param time ROC曲线生存时间节点，单位为年，默认为c(1, 3, 5)
#' @param timecol 生存时间对应的列名
#' @param statuscol 生存状态对应的列名
#' @param color_fun 字符向量，用于样本高低分组的颜色
#' >color_fun1
#' "#377EB8" "#E41A1C" "#4DAF4A" "#FF7F00" "#984EA3" "#F781BF" "#A65628"
#' @param surtime_unit 数值，转换成year的时间刻度，比如说输入的临床信息表里是天，这里则写365、月则写30
#' @param no_roc_pheatmap 逻辑字符，是否需要单独出不包含ROC和heatmap的拼图，默认为FALSE
#' @param xlab 字符串，生存曲线x轴坐标，默认为"days"
#' @return *list*
#' @examples model_res <- modelscore_km_roc(signature_coef = lasso_res, od = out_dir, exp = data_exp, clin = data_clin_surv, dataset = "TCGA", time = c(1, 3, 5), timecol = "OS.time", statuscol = "OS", color_fun = color_fun1)
#' @author *CY*
#'
v_modelscore_km_roc <- function(signature_coef = NULL, od = NULL, exp = NULL, clin = NULL, best_cut = FALSE,
    ...
    if (length(exists_sign) > 0) {
        ...
        # 分组
        if (best_cut) {
            sur.cut <- surv_cutpoint(data = dat, time = "time", event = "status", variables = "Score",minprop=0.3)
        ...
        if (length(unique(Group[, 1])) > 1 & sum(summary(Group[, 1]) > 5) == length(unique(Group[, 1]))) {
            ...
            res <- ggsurvplot(data_plot,
                data = dat, conf.int = FALSE, pval = T, conf.int.style = "step", legend = "top", legend.title = dataset,
