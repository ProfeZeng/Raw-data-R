# /Pub/Users/wangyk/Project_wangyk/Codelib_YK/some_scr/NewLover/ClinicalFeatures_plot.R
#'
#' @TODO 临床特征分布差异
#' @title ## 临床特征分布差异
#' @description 可以出两份图，一份是boxpolt得分在不同临床信息中的分布，另一份是分组分布柱状堆积图，样本不同分组在临床信息中的分布，按输入数据分别或者一起出图.
#' @param Data 输入的数据，可以是data.frame
#' @param phenotype_col *string vector*，各种临床信息在`Data`中所在列名，字符串向量
#' @param score_col *character*，得分所在列，在`Data`中所在列名，字符串向量，如果为NULL，则不进行socre分布分析
#' @param group_col *string vector*，分组所在列，在`Data`中所在列名，字符串向量，如果为NULL，则不进行样本特征分布分析  
#' @details 本质上，`group_col`是x轴变量，`phenotype_col`是y轴变量，
#' 如果`group_col`数量大于1且`phenotype_col`数量等于1则绘制的是，`phenotype_col`在不同的`group_col`分布情况
#' @param var_name *character*，变量名，用于命名输出文件夹，如果为`NULL`，则使用四个随机字母
#' @param width_single *numeric value*，出图时，单图宽度；合图宽度是单图宽度的整数倍 
#' @param height_single *numeric value*，出图时，单图高度；合图存在多行时，高度是单图高度的整数倍 
#' @param color_used *string vector*，出图使用配色，字符串向量。如果为`NULL` 则默认使用`RColorBrewer::brewer.pal(8,'Set2')`配色
#'
#' @param saveplot *logical value*，是否输出图片到`output_dir`
#' @usage
#' a <- score_in_clinical_model(Data = data_12313, phenotype_col = 'riskgroup', score_col = NULL,
#'                              group_col = c("age","gender","stage"), 
#'                              output_dir = output_dir, var_name = 'pheno_risk123',
#'                              width_single = 3.6, height_single = 4.2, color_used = col_12, saveplot = T)
#' a <- score_in_clinical_model(Data = data_12313, phenotype_col = c("age","gender","stage"), score_col = NULL,
#'                              group_col ='riskgroup' , 
#'                              output_dir = output_dir, var_name = 'pheno_risk123',
#'                              width_single = 3.6, height_single = 4.2, color_used = col_12, saveplot = T)
#' 
