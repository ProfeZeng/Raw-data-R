...
#' TCGA-VN-A88O-01A     A
#' @param od 向量，结果输出路径
#' @param feature2show 需要展示的feature字符串向量；或 数字，显示最显著的前 数字 个,默认是"all"，显示全部feature
...
v_characteristics_plot_by_group <- function(characteristics_score = NULL, Group = NULL, 
    od = NULL, color_fun = c("#b6dacb", "#f2c3aa", "#cac9e1", "#c6ddb0", "#f6dead", "#ddcbaf"),feature2show="all",
    ...
    sign_stat <- sapply(characteristics_list, function(x) {
        ...
        if (length(unique(Group$Group)) > 2) {
            ...
            p <- wilcox.test(value ~ Group, data = data)$p.value
        }
        return(p)
    })
    sign_stat <- sign_stat %>%
